...
| promptfoo | 一切可验证（评测执行器） | 机制章新增；24.5b 自研形状的执行器一侧 | 第 24 章 24.5c · promptfoo 与 garak | **B** |
| garak | 一切可验证（红队探针） | 机制章新增；附录 J 三层 eval 的 L3 红队行 | 第 24 章 24.5c · promptfoo 与 garak | **B** |
| Langfuse | 文档即代码（提示词宿主） | 机制章新增；案例三/四的链路版本化侧 | 第 23 章 23.4f | **B** |
| LangGraph | 文档即代码（链路即留痕） | 案例四 0.2 状态图（4 月 25 日越界那节的宿主栈） | 第 23 章 23.4 · LangGraph | **B** |
| tiktoken | 契约先行（上下文预算） | 机制章新增；第 1 章 1.5c 预算表的分词器口径那一半 | 第 1 章 1.5c · tiktoken | **B** |
...
4.5c 那张台账自然会招出一个问题：点名的这些工具里，A、B、C、规范、形状各占几件，哪根支柱最薄？手工点数不可信——台账本身就是抄件，抄件数出来的数又抄进正文，是二次腐烂。这一节的办法是跑把这张表对账的那个守卫，让它自己打印，计数命令也一并钉死在正文里；**台账有几行、各档有几件，只有一个宿主：下面那几条命令打印出来的行**，本节正文里出现的每个数都是这一次跑出来的，不是从别处抄来的常量。**本节数字与四个案例无关、不进数字清单**：它是本仓库守卫的输出，A 档口径指"本仓库、本机、可复核"——命令原样贴在下面，输出逐字抄回，一个字没改。

第一跑，对账闸本身（退出码 0）：

```bash
$ python3 scripts/check_tier_ledger.py
[覆盖] 台账 54 行（解析 54 行）；卡上带键声明 54 条；逐行对上 54 行；落点查无文件 0 行；孤立声明 0 条
台账闸通过：54 行档位全部由对应卡片自己那行声明支撑，键整串相等、落点相符。
[盲区] 本闸只判「台账 == 卡上写的」；卡上写的是否 == 机器上发生的，仍要人读那张卡。
```

第二跑，`--print` 把整列档位从卡上派生出来；下面是它的第一行与最后一行（中间的行从略，最后一行是派生覆盖读数）：
...
[派生] 可由卡派生 54 行；派生不出来（缺声明／落点不符）0 行——这些格子不许手填字母；档位括号外的理由是人写的，不参与对账
```
...
把两跑读数合起来读：54 件点名的工具里，**纯 A 14 行、A/B 两半 5 行、纯 B 25 行、规范档 6 行、形状档 2 行、C 2 行**；带 A 半边的是 19 行（14 + 5），带 B 半边的是 30 行（25 + 5），两式都由上面那列读数直加，不是手抄别处。`--print` 末行那句"派生不出来 0 行"是给这些数兜底的：每一格都是从卡上长出来的，没有一格是手填的字母。**这一节被复跑的那一天，台账刚从 53 行涨到 54 行**（新增一件：第 23 章 23.4 的 LangGraph，B 档；上一轮涨到 53 行时新增的四件是 promptfoo、garak、Langfuse、tiktoken）——上面每一行读数都是涨完之后重跑的，改完台账不重跑本节，正文就与守卫输出不符，这是这一节刻意把命令贴全的原因。同一天还顺手钉了一颗只有人读才抓得到的雷：第 23 章里出现过**两个都编号 23.4b 的小节**，第十二条守卫按键对账，两个"23.4b"它一个都不碰——节号撞号不在它的判据面上，Langfuse 那节已人工改号为 23.4f，落点、附录 J 的引用与本节的历次计数引用同步跟上。

档位列回答"各几件"，支柱列回答"哪根薄"。支柱口径直接取 4.5c 台账表第二列，另起一条 awk 数出来（54 行，行序按 sort）：
...
契约先行 行 11 含A 5 含B 6 C 0 规范 1 形状 0
文档即代码 行 7 含A 2 含B 5 C 0 规范 0 形状 0
一切可回溯 行 10 含A 2 含B 6 C 0 规范 2 形状 1
一切可验证 行 26 含A 10 含B 13 C 2 规范 3 形状 1
```

读数三条，逐条对着上面的行说。**第一条：「一切可验证」是唯一一根能直接抄着用的支柱**——26 行、含 A 10 行，厚度和 A 档密度都排第一。原因不在运气，在这本书的结构：这根支柱的多数卡是机制章（第 5、6、10、13 章）自己在本机跑出来的。
...
**第三条：「文档即代码」行数垫底**（7 行），但 7 行里 2 行含 A——检索地基那两张卡（第 7 章的切块与预算算术）恰好是跑得动的那部分，而真正当门用的"留痕检查"反而一无所有（4.5d 第⑤步）。所以**支柱的厚薄不看行数，看行数在"跑得动、只能读、只能画"三级台阶上的分布**——行数只是卡数的影子。
