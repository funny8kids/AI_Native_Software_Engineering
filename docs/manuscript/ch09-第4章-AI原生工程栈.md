...
|---|---|---|---|---|
| OpenAPI / spec 校验 | 契约先行 | 案例一、第 17 章用 HTTP 契约 | 第 9 章 | **A** |
| Protobuf / protoc | 契约先行 | 案例一的 gRPC 与 RPC 契约 | 第 9 章 | **A** |
| nginx | 契约先行（路由即边界） | 案例一的入口层 | 第 13 章 | **A** |
| import-linter | 可验证（架构不变量） | 案例一、案例三 | 第 8 章（全卡）· 第 6 章（判据行） | **B** |
| Semgrep | 一切可验证 | 案例四用它扫代码 | 第 22 章 | **B** |
| Trivy | 一切可验证 | 案例四用它扫容器 | 第 22 章 | **B** |
| Prometheus | 一切可回溯 | 案例一、二、四的 metrics 主力 | 第 21 章 | **B** |
| Grafana | 一切可回溯 | 案例一、二、四的面板层 | 第 21 章 | **B** |
...
| Sentry | 一切可回溯 | 案例二一句「追踪错误」 | 第 21 章（本轮从 C 档升上来：判据行取到了出处，但**在源码不在文档**） | **B** |
