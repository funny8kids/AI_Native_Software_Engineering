...
三个 30 天的节奏画成甘特，里程碑状态一目了然（日期为示意，与总路线图同口径）。图 4-3 按月分三节、每节两条任务条：第一月先落「契约仓库最小集与 Owner 具名」、再把契约 diff 挂成硬门；最末一条「ADR 与季度自检挂钩」正是 4.9 里让自检表不沦为形式的那个挂钩：
...
| entry points（`importlib.metadata`） | 契约先行（接缝的契约义务） | 机制章新增 | 第 11 章 | **A** |
| 进程内注册表与 SPI | 契约先行 | 机制章新增 | 第 11 章 | **A** |
| import-linter | 一切可验证（架构不变量） | 案例一、案例三 | 第 8 章（全卡）· 第 6 章（判据行） | **B** |
...
| p50/p99 分位算术（为什么不能相加平均） | 一切可验证 | 案例一、二 | 第 18 章 18.6g | **A** |
...
| required checks 与分支保护绕过面 | 一切可验证 | 机制章新增 | 第 19 章 19.6d | **A**（含本机 git 侧对照） |
...
| Prometheus / Grafana | 一切可回溯 | 案例一、二、四的 metrics 主力与面板层 | 第 21 章 | **B** |
| Redis | 一切可回溯（容量线） | 案例一/二/四的 7.x Cluster | 第 21 章 | **B** |
| Kafka | 一切可回溯（事件流） | 案例一、二写的是 3.6 | 第 21 章 | **B** |
| Sentry | 一切可回溯 | 案例二一句「追踪错误」 | 第 21 章（曾判 C，判据行取自**源码不在文档**） | **B** |
| Argo Rollouts 的 steps 与 analysis | 一切可回溯（渐进交付） | 机制章新增 | 第 28 章 28.5b | **形状档**（本书未跑） |
| Semgrep | 一切可验证 | 案例四用它扫代码 | 第 22 章 | **B** |
| Trivy | 一切可验证 | 案例四用它扫容器 | 第 22 章 | **B** |
| 自研秘密扫描三判据（`secscan.py`） | 一切可验证 | 机制章新增 | 第 22 章 22.6g | **A**（同一条命令的命中数不幂等，见卡内说明） |
| AI 生成代码的额外扫描规则 | 一切可验证 | 案例四 | 第 24 章 | **B**（本机没有 semgrep，只给规则形状） |
| Mythril / Slither | 一切可验证（合约侧静态分析） | 案例三闸门二的第 2、3 层 | **无卡**：本机未装、本轮未取到判据行 | **C** |
| Foundry（forge / anvil） | 一切可验证（合约侧） | 案例三的 `forge test` / `forge coverage` / `anvil` fork | **案例三**（见下方例外说明） | **B** |
...
| LangChain | 文档即代码（链路即留痕） | 案例三自研 workflow、案例四 0.2 | 第 23 章 | **B** |
| W3C Design Tokens 字段口径 | 文档即代码 | 机制章新增 | 第 15 章 15.7b | **B** |
| 视觉回归工具链（截图比对） | 一切可验证（多端一致） | 机制章新增 | 第 15 章 | **形状档**（本机未运行） |
