...
| Langfuse | 文档即代码（提示词宿主） | 机制章新增；案例三/四的链路版本化侧 | 第 23 章 23.4b | **B** |
...
[覆盖] 台账 53 行（解析 53 行）；卡上带键声明 53 条；逐行对上 53 行；落点查无文件 0 行；孤立声明 0 条
台账闸通过：53 行档位全部由对应卡片自己那行声明支撑，键整串相等、落点相符。
...
[派生] 可由卡派生 53 行；派生不出来（缺声明／落点不符）0 行——这些格子不许手填字母；档位括号外的理由是人写的，不参与对账
```
...
把两跑读数合起来读：53 件点名的工具里，**纯 A 14 行、A/B 两半 5 行、纯 B 24 行、规范档 6 行、形状档 2 行、C 2 行**；带 A 半边的是 19 行（14 + 5），带 B 半边的是 29 行（24 + 5），两式都由上面那列读数直加，不是手抄别处。`--print` 末行那句"派生不出来 0 行"是给这些数兜底的：每一格都是从卡上长出来的，没有一格是手填的字母。**这一节被复跑的那一天，台账刚从 49 行涨到 53 行**（新增四件：第 24 章 24.5c 的 promptfoo 与 garak、第 23 章 23.4b 的 Langfuse、第 1 章 1.5c 的 tiktoken，四张全是 B 档）——上面每一行读数都是涨完之后重跑的，改完台账不重跑本节，正文就与守卫输出不符，这是这一节刻意把命令贴全的原因。

档位列回答"各几件"，支柱列回答"哪根薄"。支柱口径直接取 4.5c 台账表第二列，另起一条 awk 数出来（53 行，行序按 sort）：
...
文档即代码 行 6 含A 2 含B 4 C 0 规范 0 形状 0
...
**第三条：「文档即代码」行数垫底**（6 行），但 6 行里 2 行含 A——检索地基那两张卡（第 7 章的切块与预算算术）恰好是跑得动的那部分，而真正当门用的"留痕检查"反而一无所有（4.5d 第⑤步）。所以**支柱的厚薄不看行数，看行数在"跑得动、只能读、只能画"三级台阶上的分布**——行数只是卡数的影子。
