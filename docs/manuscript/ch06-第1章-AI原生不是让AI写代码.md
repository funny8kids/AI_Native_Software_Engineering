...
## 1.5d 生效证据清点：边界清单的每一条挂在哪个产物上

1.5 给了清单的三分类，图 1-3 给了它的机器判定，代价声明给了一个**整体**读数——发布后 AI 生成提交量降约 22%。但整体读数验的是清单「在不在生效」，不验**哪一条**在生效：一次签字批下去三类边界，其中有没有某条从签字那天起就没拦过任何东西，22% 这个数回答不了。清点要逐条做，每条只登记三态：**有门禁产物、产物可造假、无产物**。

| 边界条目（出处） | 判定产物 | 这条证据怎么读 | 没有产物时怎样 |
|---|---|---|---|
| 禁入 · 支付对账、风控规则（1.5，案例一） | 流水线里按路径归属判定的拒绝记录，拒绝原因点名条目 | 拒绝数不为零，且理由字段指向清单原文——拦了且说清拦的是哪条，才算这条边界活着 | 清单退回墙上：1.7 反模式 6「只在墙上不在 CI 里」的逐条形态；1.1 那次限流事故之所以贵，就是因为没有任何产物替那条链路说话 |
| 禁入 · 资金计算精度（案例二） | 不变量门的可执行断言与红灯消息（1.5b 第三道门） | 断言消息能点名被破坏的约束——1.5b 本机实跑那条 `45.0` 的形状；精度区若一条断言都没有，这条边界就只是措辞 | 定点数换浮点数这类变更撞不上任何门，差额要等到对账表上才浮出来——读数迟到，代价照付 |
| 禁入 · 合约部署、策略上线准入（案例三） | 评测门的阈值记录与关键样本回退记录（1.5b 第四道门） | 回退记录出现在上实盘**之前**；只有事后归因没有事前红灯，说明这道门挂在了发布之后 | 1.1 那句「回测惊艳、实盘亏损」就是这条边界无产物时的标准叙事——惊艳和亏损之间没有一格机器读数 |
| 禁入 · 智能体提权、跨体变更（案例四） | 行动拒绝日志：权限被请求又未被满足的记录 | 日志里至少要有「被尝试」这一列——连尝试都没有，无法区分「边界挡住了」和「没人拿它测过」 | 1.1 那个改签名破坏另一智能体规则基准的变更，当时接住它的是事后回滚，不是任何一份拒绝日志 |
| 需审批 · 查询接口、非关键链路（1.5） | 具名签字的选择记录（图 1-3 中间出口） | 提交量与签字计数同向才叫审批在跑；提交涨、签字不涨，签字框在变橡皮章 | 「需审批」滑成「顺便知会一声」，清单的中间出口变成旋转门 |
| 可碰 · 配置代码、展示组件（1.5） | 自动放行日志，且同域契约 diff 保持绿 | 可碰域里若冒出契约破坏，是**归类错了**不是门松了——退回清单重标 | 最隐蔽的一条：可碰标签会顺着惯性扩到相邻链路，12 个业务域的清单里，边界外新增的域比边界内漏网的提交更危险，因为它走的是自动放行 |

这四列是咬合的：条目没有产物，条款退化为立场声明；产物没有读法，日志只是磁盘占用；「没有产物时怎样」这一列不许留空——空了它就等于清点只给活条款发奖状，而清点的全部价值恰恰在给死条款开死亡证明。三态里最该被点名的是「产物可造假」：若路径归属由提交人自报（AI 生成的改动被手工标成「配置代码」就能走可碰出口），门禁拦住的只是不肯撒谎的人。判定的事实源必须机器可得——按仓库路径、按调用方清单，而不是按声明字段。

22% 这个整体读数在清点里另有一用：各条禁入边界的被拒提交数加起来，量级上应能解释这次下降。若清单发布后的观察期已经拉得和 1.1 里量提交量涨幅（35%）的那两个月一样长，而被拒总数远小于 22% 对应的量，那么至少有一条边界没牙——下降要么是被清单外的东西拦的，要么是审批恐惧让一线干脆不提交了。第一种要补产物，第二种比不降还糟：它意味着速度回了本，刹车却没装上，1.1 那个「事故率涨 60%」的组合就还在等着。

---

## 1.5e 决策点表：四类拒绝点各自谁判、拒了怎么办、到什么时候

1.5b 造了四道门，又用三态清点回答「门在不在」。门立起来之后，下一件事自己就送上门：门每次亮红灯，都是一个待处理的拒绝。红灯之后怎么办，1.5b 没写——没有判据、动作、签署方、时限的红灯，和没有红绿灯的路口一样危险，只是危险的方式不同。本节把四类拒绝点的读数逐个挂上动作链。读数全部出自 1.5b 与 1.5c，本表不新增任何读数；签署方用 1.4 已有的椅子——清单发布时委员会还没影子（那是第 26 章的终点），本章只把动作挂到今天已经有立场的人身上。

| 拒绝点读数（出处） | 上表判据（什么形状才算触发） | 触发后的动作 | 签署方（1.4 的椅子） | 时限锚 |
|---|---|---|---|---|
| 编译/类型红灯：未定义符号、签名不匹配（1.5b 第一道门） | 同一变更链上首亮 | 打回生成侧，不进人的评审队列——人的时间留给「对不对」，不留给「存不存在」 | **本行故意不签**：编译判定无争议，让人在编译器旁边签字只会批量生产表演性签字 | 复核随当日站会，不拖过评审积压那条 ≤ 1 天的线（26.5 效率类目标） |
| 契约红灯：字段被删、枚举缩窄（1.5b 第二道门） | 破坏性 diff 亮红，且同期内消费方无同步记录 | 冻结合并候选；消费方同步立成工单；边界本身被质疑时重裁归类 | 多端负责人——1.4 写死了他的条件：碰到跨端契约必须经过审批 | 跨域契约的破坏性变更按三类必上事项上会（26.5） |
| 不变量红灯：断言消息点名业务红线（1.5b 第三道门，本机实跑 `45.0` 形状） | 红灯带原因，且原因落在清单某条业务约束上 | 先裁约束本身还成不成立——互斥是取消还是保留是**业务决策不是 bug**，裁完才轮到改代码或改断言 | 风控——1.4 里他对资金链路有一票否决；时限沿用 26.4 的「X 天」占位，超时默认同意 | X 天（26.4 占位，本地标定，本书不填样板答案） |
| 评测红灯：分数低于基线、关键样本回退（1.5b 第四道门） | 红灯，且样本集自上一次复审以来未更新 | 先复核样本集覆盖度，再谈阈值——基线过期时红灯量的是旧世界，改样本集优先于改门限 | 治理推动者——1.4 里他是唯一「功劳归决策层、锅归自己」的椅子，这道最容易被省的门只能他签 | 评测基线复审挂月会（第 24 章的门禁口径） |
| 图 1-4 全绿（四道门皆过） | 全绿且被当作「可以合并」 | 全绿只是拿到进入人工卡点的资格——方向盘动作另行发生，不在此表任何一行 | 具名的人，按[第 2 章](./ch07-第2章-人在回路.md)的人在回路卡点签 | 不越评审积压线（同上，≤ 1 天） |

为什么这四列咬合：判据决定红灯值不值得上表，动作决定它不是白亮，签署方决定门不会被「谁有空谁管」耗死，时限决定签字框不会变成永久刹车。缺哪一列都会复发一种本写过的病——缺判据，1.5b 的三态清点里它就停在「亮黄灯」；缺签署方，第二层「出事靠人临时兜底」（1.3）就是缺签署方的官方名字；缺时限，否决会烂成 1.4 里风控那个零损失立场的无限期不表态，那是第 26 章反模式七在本章的前身。

编译行留空签署方，是这张表最容易被误读的一格，值得再多说一句：留空不是遗漏，是把「人必须出现在每个环节」这种合规幻觉挡在门外。1.4 的一线工程师要的是速度，编译器拒的提交本来就该原地打回——一旦允许人签，凌晨三点合并的人就多了第三条合法通道（1.7 反模式 5 说透了：自觉拦不住凌晨三点）。

这条从红灯到复审的链画成图 1-5，两个菱形是它的全部关节：

```mermaid
flowchart LR
  D["门亮红灯<br/>带原因"] --> C{归到清单条目了吗}
  C -->|归到| A["按出口处置<br/>打回 / 冻结 / 裁约束 / 复核样本"]
  C -->|归不到| L["进异常清单<br/>域标注与清单分叉"]
  A --> S{时限内具名签署?}
  S -->|超时未签| G["默认同意<br/>复审排进档期"]
  S -->|签或否| R["附反对意见原文<br/>进复审"]
  R --> G
  G -.读数复查到期.-> D
  style D fill:#ffe3df,stroke:#a03b31,color:#1e1c19
  style C fill:#fcefd3,stroke:#9d6127,color:#1e1c19
  style S fill:#fcefd3,stroke:#9d6127,color:#1e1c19
  style G fill:#e2f3df,stroke:#3e7247,color:#1e1c19
```

**图 1-5｜拒绝点的决议链** — 归不到条目的红灯不拦提交但必须留档；超时未签走默认同意，复审到期再量一遍。

读图 1-5 盯三处。第一，「归不到」分支不拦任何提交——1.5d 最底下那行说过，可碰标签顺着惯性扩到相邻链路时，走的正是这条出口；它的全部力气在「留档」，异常清单攒起来就是清单改版的第一素材。第二，「超时未签 → 默认同意」看着像漏洞，实为刹车与否决权的对价：没有时限的门会把 1.4 一线工程师的速度诉求碾成合规停车场，第 26 章为这条对价专门立了一条反模式，本章在清单刚发布的阶段就要认它的账。第三，虚线回边是全图唯一不产生新动作的走线——默认同意不等于结案，同一条读数到期还要被这道门再拒一遍；砍掉这条回边，本节和 1.5b 的一切都会退化成一锤子买卖。

---

## 1.5f 滑坡判据：三层能力先从哪条缝退回去

1.3 的那一问（流程里有没有专为 AI 出错设计的刹车）判定的是姿态，1.5b 的三态清点切的是当时的截面。但层级不是爬上就保住的——往回滑有方向，而且先露出来的从来不是「我们退回第一层了」这种自我认知，是六条具体产物上的形状。逐条登记，每条的证据面只用 1.5 到 1.5e 已经造好的产物，不新设信号源。

| 滑坡信号 | 先看见的形状（证据面） | 烂掉的是哪个机制 | 反向动作 |
|---|---|---|---|
| 门黄灯化 | 流水线配置里该步骤被设成 warning、`continue-on-error` 一类；CI 面板上这一步仍显示「存在」 | 1.5b 三态的第 3 态滑回第 2 态 | 要么改回阻断要么删掉这一步——挂着不拦是全表最坏的配置，它骗过清点还骗过复盘 |
| 门挪出合并前 | 拒绝记录的时间戳集中到夜间批量时段 | 1.5b 引的第 10 章那句「晚报」 | 把时间戳挪回合并前即可，不需要新门——这一格是六条里动作最便宜的，也是最常被「先保发布节奏」让掉的 |
| 红灯丢原因 | 退出码非零，消息只剩一行计数；断言不带被破坏约束的名字 | 1.5b 实跑纪律的反面 | 先修输出格式再谈别的——没有原因的红灯，下一轮修订没有抓手，复审链（图 1-5 第一个菱形）当场断掉 |
| 清单空洞 | 新链路的 AI 提交匹配不到清单任何一条；1.5d 清点出现「无产物」行 | 1.5 的边界清单 | 空洞二选一：划入准入或划入禁入，两边都得签字——放任空洞等于允许清单只盖住昨天的系统 |
| 域标注自报 | 可碰域自动放行占比上升，同域契约 diff 红灯同向上升 | 1.5d 的「产物可造假」态 | 判定事实源改回路径归属；放行率与破坏率同涨的那一刻，可碰标签已经在替人撒谎 |
| 降幅回弹、事故未降 | AI 生成提交量爬回清单发布前水平，同时 1.1 那组读数（速度、事故率）没有同向回落 | 1.5 的代价声明、1.2 的第三条根因 | 这是最重的一格：两个数**反向**回到原位说明清单只剩下名字——「挂着原生之名行辅助之实」在这一格里有了可核对的定义 |

把这六条放回 1.1 的四个事故上走一遍反事实，滑坡判据的用处就清楚了：限流延时被删那条提交，若有不变量门且红灯带原因，**打回发生在合并之前而不是封禁发生在上游之后**；定点数换浮点数，先浮出来的应该是精度断言的第一次红灯，而不是对账表上数百万的差额；回测惊艳的趋势策略，关键样本回退应该出现在准入出口，而不是实盘亏损之后；改签名破坏规则基准的修复智能体，消费方 diff 应该先点名跨体变更——四个案例的损失结构完全一致：**凡是本该早于损失出现的读数，都被推到了损失之后。** 从第三层滑回第一层，滑的就是这个先后次序。

最后记一笔诚实账。这六条信号量的是 1.5 到 1.5e 的产物，也就是说，若有一天清单只剩签字、门禁一件产物都不再产生，本表会永远报绿——证据面为空时，「无异常」和「无证据」是同一个形状，1.5c 那句纪律在这里同样成立：复述不出来的读数不采信。所以这张清点的最低执行动作，不是打勾，是每次跑完把 1.5d 那份「有门禁产物、产物可造假、无产物」的三态结论留档，附进 1.6 检查清单之后；一套读数永远干净、从没亮过红的清点，和一条从没拒过任何提交的流水线一样，干净得可疑。
