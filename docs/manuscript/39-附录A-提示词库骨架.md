...
role_conflict: 阿杰希望评低风险以加快交付，林工希望资金域评高风险
status: skeleton
```
...
must_intervene: 契约须契约 Owner（多端 TL）签字方可入仓库
ai_failure_mode: 漏列错误码、把破坏性字段标成兼容（对应 B-0527/E-0311 风险）
role_conflict: 小王要求破坏性变更须他签字
status: skeleton
```
...
role_conflict: 阿杰想最小化留缝，林工要求资金链路强制留
status: skeleton
```
...
must_intervene: 资金相关缺口不补不让上；林工签字
status: skeleton
```
...
prompt: |
  ...
  并列出受影响端与建议排期。
must_intervene: 破坏性须小王签字（2 工作日时限）
status: skeleton
```
...
id: P-29-7-01
chapter: 29.7
scene: CTO 大会复盘，把三次失灵归因
objective: 输出"失灵—接手"对照
prompt: |
  把 D-0114 / E-0311 / F-0515 三件事整理成"AI 失灵点 → 组织谁接手 → 接手方式 → 代价"的对照表。
...
prompt: |
  基于 <数字清单>，生成治理前后对比表，标注每个数字的数据来源与置信度。
