...
```text
手稿 43 个文件 / 175 张图；站点页 3 张；合计 178 张。
正文图引用 296 条 / 全书图注 178 条（唯一编号 178 个）/ 悬空与撞号 0 条。
BOOK_SPEC §9 校验通过：每章 2–5 张、图号递增、图注与图一一对应、节点 ≤12、正文图引用全部可解析、图注编号全书唯一。

```

42 个 `chNN-` 文件全部落在 2–5 张区间：2 张 4 个、3 张 5 个、4 张 14 个、5 张 19 个（顶到 §9 上限的是第 1、3、4、9、16、18、19、20、21、23、25、26、27、28 这 14 章，加上四个案例章与附录 J，共 19 个文件）。**这一句由那张表派生，不再手抄**：2026-09-26 第九轮起，重打这一节的脚本直接从 `check_figures.py --print` 的输出里数分桶、点顶格章，改完正文只需重跑那条命令与这个脚本，两边一起动。为什么值得为它做一次派生：这一句在手抄的年代就错过账——同日拿同一口径回量 HEAD 那一份表，3 张那一桶表里是 7 个文件，句子里写着 8 个；差一只桶的整句作废，重派即对。（2026-09-25 补附录那四份文件各加 1–3 张图，手稿从 150 涨到 159，随后案例四再补两张到 161、案例一与案例三各补两张到 165、第 26 章组织治理再补两张到 167（2026-09-26）、第九轮三章各补一到两张到 171、第十轮第 10 章与第 25 章各补一张到 173、第十一轮第 11 章与第 16 章各补一张到 175（同日），每一次都是先重跑命令再改这两处读数的）。`manuscript/README.md` 是书稿总览页（1 张骨架图），九个 `part-*.md` 卷首页用位图艺术开场——两者都不是章，不参与「2–5 张」判定，由守卫按同一口径跳过。
